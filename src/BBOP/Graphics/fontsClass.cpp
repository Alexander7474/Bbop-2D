...
void loadFontTexture(const char *fontPath, int fontSize, Character *charList,
	int numTextures)
{

	FT_Library ft;
	if (FT_Init_FreeType(&ft) != 0) {
		std::cerr << "Erreur lors de l'initialisation de FreeType"
			  << std::endl;
		return;
	}
	...
	if (FT_New_Face(ft, fontPath, 0, &face) != 0) {
		std::cerr << "Erreur lors du chargement de la police de "
			     "caractères"
			  << std::endl;
		FT_Done_FreeType(ft);
		return;
	...
	if (FT_Set_Pixel_Sizes(face, 0, fontSize) != 0) {
		std::cerr << "Erreur lors du redimensionnement de la police de "
			     "caractères"
			  << std::endl;
		FT_Done_Face(face);
		FT_Done_FreeType(ft);
	...
	glPixelStorei(
		GL_UNPACK_ALIGNMENT, 1); // Désactive le padding pour la texture
	...
	for (int i = 0; i < numTextures; ++i) {
		...
		if (FT_Load_Char(face, character, FT_LOAD_RENDER) != 0) {
			std::cerr << "Erreur lors du chargement du glyphe "
				  << character << std::endl;
			continue;
		}
		...
		// Inverser le glyph
		for (unsigned int row = 0; row < face->glyph->bitmap.rows / 2;
			++row) {
			for (unsigned int col = 0;
				col < face->glyph->bitmap.width; ++col) {
				std::swap(face->glyph->bitmap.buffer[row *
							  face->glyph->bitmap
								  .width +
						  col],
		...
		for (unsigned int row = 0; row < face->glyph->bitmap.rows;
			++row) {
			for (unsigned int col = 0;
				col < face->glyph->bitmap.width; ++col) {
				unsigned char red =
					face->glyph->bitmap.buffer[row *
							face->glyph->bitmap
								.width +
						col];
				unsigned char *pixel =
		...
		charList[i].bearing.x =
			face->glyph
				->bitmap_left; // Conversion de la valeur en pixels
		charList[i].bearing.y =
			face->glyph
				->bitmap_top; // Conversion de la valeur en pixels
...
TexteBox::TexteBox(const char *nTexte, Font *nFont)
    : texteFont(nFont), pos(0.0f, 0.0f), origin(0.0f, 0.0f), rotation(0.0f),
      RGB(255, 255, 255), alpha(1.0f), texte(new char[strlen(nTexte) + 1]),
      sizeTexte(static_cast<unsigned int>(strlen(nTexte) + 1)),
      glyphList(new NoTextureSprite[strlen(nTexte) + 1]), offset(2.5f)
{
...
TexteBox::TexteBox(const TexteBox &other)
    : texteFont(other.texteFont), pos(other.pos), origin(other.origin),
      rotation(other.rotation), RGB(other.RGB), alpha(other.alpha),
      texte(new char[other.sizeTexte]), sizeTexte(other.sizeTexte),
      glyphList(new NoTextureSprite[other.sizeTexte]), offset(other.offset)
{
...
void TexteBox::buildTexteBox()
{
	if (texteFont == nullptr) {
		std::cerr << "Error buiding texteBox, font initialized has "
			     "nullptr"
			  << std::endl;
		return;
	}
	...
	float sizeY = 0.f;
	for (unsigned int i = 0; i < sizeTexte; i++) {
		glyphList[i].setSize(Vector2f(
			texteFont->charL[static_cast<unsigned char>(texte[i])]
				.size.x,
			texteFont->charL[static_cast<unsigned char>(texte[i])]
				.size.y));
		if (glyphList[i].getSize().y > sizeY)
			sizeY = glyphList[i].getSize().y;
		glyphList[i].setOrigin(Vector2f(
			-(sizeTotal +
				texteFont
					->charL[static_cast<unsigned char>(
						texte[i])]
					.bearing.x) +
				origin.x,
			texteFont->charL[static_cast<unsigned char>(texte[i])]
...
Vector2f TexteBox::getPosition() { return pos; }
...
Vector2f TexteBox::getOrigin() { return origin; }
...
Vector3i TexteBox::getColor() { return RGB; }
...
float TexteBox::getAlpha() { return alpha; }
...
float TexteBox::getRotation() { return rotation; }
...
const Vector2f &TexteBox::getSize() { return size; }
