...
AnimatedSprite::AnimatedSprite()
    : AnimatedSprite("assets/animated_sprites/Smoke-Sheet.png", Vector2i(4, 5),
	      0.1f, 1)
{
}

AnimatedSprite::AnimatedSprite(
	std::string _animated_sprite_sheet, Vector2i _size, float _frame_t)
...
AnimatedSprite::AnimatedSprite(std::string _animated_sprite_sheet,
	Vector2i _size, float _frame_t, int _dead_frame)
    : AnimatedSprite(_animated_sprite_sheet, _size, _frame_t, _dead_frame, true)
{
}

AnimatedSprite::AnimatedSprite(std::string _animated_sprite_sheet,
	Vector2i _size, float _frame_t, int _dead_frame, bool _random_start)
    : Sprite()
{
	animated_sprite_texture = bbopLoadSpriteSheet(
		_animated_sprite_sheet.c_str(), _size.y, _size.x);
	frame_t = _frame_t;
