...
int bbopInit(
	int windowX, int windowY, const char *windowName, GLFWwindow *&window)
{
	//Initialisation Début ############################################################################
	// Initialisation de GLFW
	if (!glfwInit()) {
		LOGS.push_back(
			"BBOP ERROR -> Échec de l'initialisation de GLFW");
		return -1;
	}
	...
	window = glfwCreateWindow(
		windowX, windowY, windowName, nullptr, nullptr);
	if (!window) {
		LOGS.push_back("BBOP ERROR -> Échec de la création de la "
			       "fenêtre GLFW");
		return -1;
	}
	BBOP_WINDOW_SIZE.x = windowX;
	BBOP_WINDOW_SIZE.y = windowY;
	BBOP_WINDOW_RESOLUTION.x = windowX;
	BBOP_WINDOW_RESOLUTION.y = windowY;
	...
	if (glewInit() != GLEW_OK) {
		LOGS.push_back(
			"BBOP ERROR -> Échec de l'initialisation de GLEW");
		return -1;
	}
	...
	helloMsg += "\nAuthor: Alexander74  \nContact: "
		    "alexandre.lanternier@outlook.fr \nLicense: GPL-3.0";
...
void bbopCleanWindow(GLFWwindow *&window, Vector3i rgb, float alpha)
{
	...
	glfwGetFramebufferSize(
		window, &BBOP_WINDOW_SIZE.x, &BBOP_WINDOW_SIZE.y);
...
void bbopChangeWindowSize(const Vector2i &nWinS) { BBOP_WINDOW_SIZE = nWinS; }
