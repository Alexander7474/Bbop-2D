...
float fast_rand(float min, float max)
{
	...
	return min +
		(seed >> 16) * (1.0f / 65535.0f) *
		(max - min); // Scale to [min, max]
}
...
int main()
{
	...
	AnimatedSprite bougie(
		"imgTesting/bougie/sprite_sheet_candle1-sheet.png",
		Vector2i(5, 1), 0.1f, 0);
	...
	Light light(Vector2f(0.f, 0.f), 0.9f, Vector3i(255, 255, 255), 1.5f,
		3.0f, 4.5f);		   // bougie
	...
	while (!glfwWindowShouldClose(window)) {
		...
		Vector2f mouseWorldPos = cam.camPosToWorldPos(
			cam.screenPosToCamPos(Vector2f(static_cast<float>(xpos),
